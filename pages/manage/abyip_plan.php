...
/**
 * ============================================================
 * File     : pages/manage/abyip_plan.php
 * Project  : SKed - Youth Profiling System for Event Management
 * Purpose  : ABYIP detail page (P12) — shared by SK (full edit, own
 *            barangay only), PPSK/DILG (view + export + download signed
 *            copy, any barangay, for oversight). Mirrors
 *            pages/manage/cbydp_plan.php's structure (sections by Center
 *            of Participation, line items added/removed one at a time).
 * ============================================================
 */
...
<head>
    ...
    <title>SKed | ABYIP CY <?php echo (int) $plan['calendar_year']; ?></title>
        ...
        <main class="main" id="main-content">
            <section class="page-header mb-4">
                ...
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                    <div>
                        ...
                        <h1 class="page-title">ABYIP CY <?php echo (int) $plan['calendar_year']; ?></h1>
                        <p class="text-secondary meta-copy mb-0">
                            ...
                            MOOE ₱<?php echo number_format($totals['mooe'], 2); ?> · CO ₱<?php echo number_format($totals['co'], 2); ?> · Total ₱<?php echo number_format($totals['total'], 2); ?>
                            <?php if (!$isEditable): ?><span class="ms-2"><i class="bi bi-eye me-1"></i>View-only</span><?php endif; ?>
                        </p>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        <a class="btn-logout-outline text-decoration-none" href="abyip_export.php?id=<?php echo (int) $plan['id']; ?>" target="_blank"><i class="bi bi-printer me-1"></i> Export</a>
            ...
            <div class="docket-panel mb-4">
                <div class="d-flex flex-wrap gap-2 align-items-center justify-content-between">
                    <form method="post" action="abyip_plan.php?id=<?php echo (int) $planId; ?>" class="d-flex gap-2 align-items-center">
                    ...
                    <form method="post" action="abyip_plan.php?id=<?php echo (int) $planId; ?>" enctype="multipart/form-data" class="d-flex gap-2 align-items-center">
                        <input type="hidden" name="csrf_token" value="<?php echo e((string) $_SESSION['csrf_abyipplan_token']); ?>">
                ...
                <?php if (!empty($plan['signed_file_path'])): ?>
                    <p class="text-secondary small mt-2 mb-0"><i class="bi bi-file-earmark-check me-1"></i>Signed copy on file: <a href="plan_file.php?type=abyip&amp;id=<?php echo (int) $planId; ?>"><?php echo e((string) $plan['signed_file_original_name']); ?></a> (uploaded <?php echo e(date('M j, Y', strtotime((string) $plan['signed_uploaded_at']))); ?>)</p>
                <?php endif; ?>
            </div>
            <?php elseif (!empty($plan['signed_file_path'])): ?>
                <div class="alert alert-info" role="alert"><i class="bi bi-file-earmark-check me-1"></i>Signed copy on file: <a href="plan_file.php?type=abyip&amp;id=<?php echo (int) $planId; ?>"><?php echo e((string) $plan['signed_file_original_name']); ?></a> (uploaded <?php echo e(date('M j, Y', strtotime((string) $plan['signed_uploaded_at']))); ?>)</div>
            <?php endif; ?>
            ...
            <div class="docket-panel mb-4">
                ...
                <?php else: ?>
                    <form method="post" action="abyip_plan.php?id=<?php echo (int) $planId; ?>" class="row g-2 align-items-end">
                        <input type="hidden" name="csrf_token" value="<?php echo e((string) $_SESSION['csrf_abyipplan_token']); ?>">
            ...
            <?php foreach ($plan['sections'] as $section): ?>
                <div class="docket-panel mb-4">
                    <div class="section-heading">
                        ...
                        <?php if ($isEditable): ?>
                        <form method="post" action="abyip_plan.php?id=<?php echo (int) $planId; ?>" onsubmit="return confirm('Remove this whole section and its line items?');">
                            <input type="hidden" name="csrf_token" value="<?php echo e((string) $_SESSION['csrf_abyipplan_token']); ?>">
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm align-middle">
                            <thead>
                                <tr>
                                    <th>Ref. Code</th><th>PPA's</th><th>Description</th><th>Expected Result</th><th>Performance Indicator</th>
                                    <th>Period</th><th>MOOE</th><th>CO</th><th>Total</th><th>Person Responsible</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo number_format((float) $item['budget_mooe'], 2); ?></td>
                                    <td><?php echo number_format((float) $item['budget_co'], 2); ?></td>
                                    <td><?php echo number_format((float) $item['budget_total'], 2); ?></td>
                                    <td><?php echo e((string) ($item['person_responsible'] ?? '')); ?></td>
                                    <?php if ($isEditable): ?>
                                    <td>
                                        <form method="post" action="abyip_plan.php?id=<?php echo (int) $planId; ?>" onsubmit="return confirm('Remove this line item?');">
                                            <input type="hidden" name="csrf_token" value="<?php echo e((string) $_SESSION['csrf_abyipplan_token']); ?>">
                    ...
                    <details class="mt-2">
                        ...
                        <form method="post" action="abyip_plan.php?id=<?php echo (int) $planId; ?>" class="row g-2 mt-2">
                            <input type="hidden" name="csrf_token" value="<?php echo e((string) $_SESSION['csrf_abyipplan_token']); ?>">
            ...
            <?php if (empty($plan['sections'])): ?>
                <div class="docket-panel text-center text-secondary py-5"><i class="bi bi-cash-coin fs-1 d-block mb-2"></i>No sections yet.</div>
            <?php endif; ?>
        </main>
    </div>
